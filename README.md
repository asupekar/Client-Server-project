...
<ol>
    <li>When users begin chatting, they must set a primary chat partner. 
        <ol>
            ...
            <li>If Client A is chatting with Client B and Client C messages Client B, Client C will automatically receive a response that Client B is alredy in chat and may not respond. However the message will still be delivered and it is up to Client B if they want to respond to Client C.</li>
        </ol>
    </li>
    <li>When Client A wants to send message to Client B and Client B is offline, Client A should receive a message that Client B is offine and to try again later.</li>
    <li>Once the away message is set for client A, any other client who wants to chat with Client A will see the SetAwayMessage and won't be able to chat with client A. It is client A's responsibility to remove the SetAwayMessage first in order to receive chat messages from other clients.</li>
    <li>If a client A has an away message set, then Client A should not be able to send message to any client</li>
...
3. Or run the commands below if you are using a Mac
...
<ul>
    ...
    <li>Launch multiple client instances using ./client HOST PORT</li>
    ...
    <li>On any client (Client A), press 3 and then set a custom away message. Now this user will not be able to be messaged.
        <ul>
            <li>On Client A, press 1 to start a chat and set your chat partner to Client B. When Client A sends a message, you should be unable to complete this action and receive an error.</li>
            ...
            <li>Now, on Client A, press 5 to return from being away.</li>
            ...
            <li>To end the chat between Client A and Client B, either client can enter "/endchat".</li>
    ...
    <li>On any client (Client A), press 1 to start a chat. 
        <ul>
            ...
            <li>Client C will automatically receive a message that Client B is busy and may not responde.</li>
            <li>Client B can respond to Client C by typing in @Client C with a message.</li>
            <li>Client C can end the chat with Client B by typing "/endchat"</li>
            <li>To end the chat between Client A and Client B, either client can enter "/endchat".</li>
            <li>Now the chat is over and both clients will be taken to the main menu.</li>
        </ul>
    </li>
